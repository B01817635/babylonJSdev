...
import {
  ...
  function getMaterial(scene: Scene){
    ...
    myMaterial.ambientTexture = new Texture("./assets/textures/grass.png.jpg", scene);
    return myMaterial;
  }
  ...
  function createSphere(scene: Scene) {
    const sphere = MeshBuilder.CreateSphere(
      "ellipsoid",
      { diameter: 0.7, diameterY: 2, segments: 5 },
      scene,
    );
    sphere.position.x = 0;
    sphere.position.y = 1;
  ...
  function createCapsule(scene: Scene) {
    const capsule = MeshBuilder.CreateCapsule("capsule",{radius: 0.5, height: 2, tessellation: 4, subdivisions: 4}, scene);
  ...
  export default function createStartScene(engine: Engine) {
    ...
    createDirectionalShadows(that.direct, that.box, that.sphere);
    return that;
  }
